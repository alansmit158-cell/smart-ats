...
const RecruiterScoring = () => {
    ...
    const handleDetectAnomalies = async () => {
        ...
        try {
            ...
            if (res.data.success) {
                ...
                toast.success(t('recruiter_scoring.toast_anomalies_success'));
                setShowAnomalies(true);
            }
        } catch (error) {
            toast.error(t('recruiter_scoring.toast_anomalies_error'));
        } finally {
    ...
    const handleGlobalScoring = async () => {
        ...
        try {
            ...
            if (res.data.success) {
                toast.success(t('recruiter_scoring.toast_global_success'));
                // Refresh applications
                const appsRes = await API.get(`/applications/job/${activeJobId}`);
                setApplications(appsRes.data.data);
            }
        } catch (error) {
            toast.error(t('recruiter_scoring.toast_global_error'));
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleUpdateStatus = async (status) => {
        ...
        try {
            ...
            if (res.data.success) {
                ...
                toast.success(status === 'Rejected' ? t('recruiter_scoring.toast_status_success_reject') : t('recruiter_scoring.toast_status_success_accept'));
            }
        } catch (error) {
            toast.error(t('recruiter_scoring.toast_status_error'));
        }
    };
    ...
    if (loading) {
        return (
            <div className="h-[60vh] flex flex-col items-center justify-center gap-6">
                ...
                <p className="text-slate-500 font-bold text-[10px] uppercase tracking-[0.4em] italic">{t('recruiter_scoring.loading')}</p>
            </div>
        );
    }
            ...
            <div className="bg-white/5 backdrop-blur-3xl p-10 rounded-[3.5rem] border border-white/10 shadow-2xl flex flex-col md:flex-row items-center justify-between gap-8 relative overflow-hidden group">
                ...
                <div className="space-y-3 text-center md:text-left relative z-10">
                    <div className="flex items-center gap-4 justify-center md:justify-start">
                        ...
                        <div>
                            <h1 className="text-3xl font-bold text-white tracking-tight">{t('recruiter_scoring.title')}</h1>
                            <p className="text-slate-500 text-[10px] font-black uppercase tracking-widest mt-1 italic">{t('recruiter_scoring.subtitle')}</p>
                        </div>
                    </div>
                </div>
                <div className="flex gap-4 relative z-10">
                    ...
                    >
                        <Zap size={20} />
                        {t('recruiter_scoring.global_scoring')}
                    </button>
                    <div className="bg-white/5 p-4 px-8 rounded-2xl border border-white/10 text-center backdrop-blur-md shadow-xl">
                        ...
                        <p className="text-[9px] font-black uppercase tracking-widest text-blue-400 mt-1">{t('recruiter_scoring.matched_nodes')}</p>
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
                ...
                <div className="lg:col-span-2 space-y-6">
                    <div className="flex items-center justify-between px-4">
                        <h2 className="text-sm font-black uppercase tracking-[0.3em] text-slate-500">{t('recruiter_scoring.strategic_ranking')}</h2>
                    </div>
                    ...
                    <div className="space-y-4">
                        {applications.length === 0 ? (
                            <div className="bg-white/5 p-24 rounded-[4rem] border border-dashed border-white/10 text-center backdrop-blur-md">
                                ...
                                <p className="text-slate-500 font-medium italic">{t('recruiter_scoring.no_applications')}</p>
                            </div>
                ...
                <div className="space-y-8">
                    <h2 className="text-sm font-black uppercase tracking-[0.3em] text-slate-500 px-4">{t('recruiter_scoring.neural_focus')}</h2>
                    <AnimatePresence mode="wait">
                        {selectedApp ? (
                            ...
                            >
                                ...
                                <div className="text-center space-y-4 relative z-10">
                                    ...
                                    <div className="space-y-1">
                                        ...
                                        <div className={`inline-flex items-center gap-2 px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest border ${getScoreColor(selectedApp.scoreMatching)}`}>
                                            <Sparkles size={12} /> {selectedApp.scoreMatching > 80 ? t('recruiter_scoring.exceptional') : selectedApp.scoreMatching > 60 ? t('recruiter_scoring.optimal') : t('recruiter_scoring.moderate')} Node
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="space-y-5 relative z-10">
                                    ...
                                    >
                                        ...
                                        {selectedApp.candidate.anomalies?.length > 0 ? t('recruiter_scoring.reinit_scan') : t('recruiter_scoring.init_scan')}
                                    </button>
                                    ...
                                    {selectedApp.candidate.anomalies?.length > 0 && (
                                        ...
                                        >
                                            <AlertTriangle size={18} /> {t('recruiter_scoring.view_alerts', { count: selectedApp.candidate.anomalies.length })}
                                        </button>
                                    )}
                                </div>
                                ...
                                <div className="pt-8 border-t border-white/5 flex gap-4 relative z-10 flex-wrap">
                                    ...
                                    >
                                        <MessageSquare size={18} /> {t('recruiter_scoring.direct_message')}
                                    </button>
                                    <button 
                                    ...
                                    >
                                        <XCircle size={18} /> {t('recruiter_scoring.terminate')}
                                    </button>
                                    <button 
                                    ...
                                    >
                                        <CheckCircle size={18} /> {t('recruiter_scoring.initiate_exchange')}
                                    </button>
                                </div>
                            </motion.div>
                        ) : (
                            ...
                            >
                                ...
                                <p className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-700 italic">{t('recruiter_scoring.select_node')}</p>
                            </motion.div>
                        )}
                    </AnimatePresence>
                </div>
                        ...
                        >
                            <div className="p-10 border-b border-white/10 bg-slate-950/40 flex items-center justify-between">
                                <div className="flex items-center gap-6">
                                    ...
                                    <div>
                                        <h2 className="text-2xl font-bold text-white tracking-tight">{t('recruiter_scoring.security_scan')}</h2>
                                        <p className="text-[10px] font-black uppercase tracking-widest text-slate-500 mt-1 italic">{t('recruiter_scoring.fiability_index')} <span className={selectedApp.candidate.scoreFiabilite > 70 ? 'text-emerald-400' : 'text-rose-400'}>{selectedApp.candidate.scoreFiabilite}%</span></p>
                                    </div>
                                </div>
                            ...
                            <div className="flex-1 overflow-y-auto p-10 space-y-10 custom-scrollbar">
                                <div className="bg-white/[0.03] p-8 rounded-[2.5rem] border border-white/5 backdrop-blur-xl relative overflow-hidden group">
                                    ...
                                    <h4 className="text-[10px] font-black uppercase tracking-widest text-blue-400 mb-4 flex items-center gap-3">
                                        {t('recruiter_scoring.neural_summary')}
                                    </h4>
                                        ...
                                        >
                                            <div className="flex justify-between items-start mb-6">
                                                <span className={`px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-[0.2em] border shadow-2xl ${
                                                    ...
                                                    {t('recruiter_scoring.severity')} {ano.severite}
                                                </span>
                                            ...
                                            <div className="p-6 bg-slate-950/60 rounded-[2rem] border border-white/5">
                                                <p className="text-[9px] font-black text-blue-400 uppercase tracking-widest mb-2 flex items-center gap-2">
                                                    <Target size={12} /> {t('recruiter_scoring.strategic_recommendation')}
                                                </p>
                            ...
                            <div className="p-10 bg-slate-950/40 border-t border-white/10 backdrop-blur-3xl">
                                <button className="w-full bg-white/5 border border-white/10 text-white py-5 rounded-[1.5rem] font-bold text-[10px] uppercase tracking-[0.2em] hover:bg-white/10 transition-all shadow-2xl active:scale-95">
                                    {t('recruiter_scoring.export_kit')}
                                </button>
                            </div>
                        </motion.div>
                    </>
