...
const CandidatePortal = () => {
    ...
    const handleApply = async (jobId) => {
        if (!hasCV) {
            toast.error("Please upload your CV first to initialize matching.", {
                duration: 4000,
                icon: '📄',
    ...
    if (loading) {
        return (
            <div className="h-[60vh] flex flex-col items-center justify-center gap-6">
                ...
                <p className="text-slate-500 font-bold text-[10px] uppercase tracking-[0.4em] italic">Initializing Candidate Node...</p>
            </div>
        );
    }
    ...
    return (
        <div className="space-y-12 pb-24 relative">
                ...
                <div className="relative bg-white/5 backdrop-blur-3xl p-10 lg:p-14 rounded-[3.5rem] border border-white/10 flex flex-col md:flex-row justify-between items-center gap-10 shadow-2xl overflow-hidden">
                    ...
                    <div className="max-w-2xl text-center md:text-left relative z-10">
                        <div className="flex items-center justify-center md:justify-start gap-4 mb-6">
                            ...
                            >
                                Candidate Dashboard v5.0
                            </motion.span>
                        </div>
                        <h1 className="text-5xl font-bold text-white mb-5 leading-tight tracking-tight">
                            Elevate your trajectory with <br />
                            <span className="bg-clip-text text-transparent bg-gradient-to-r from-blue-400 to-indigo-400 italic font-medium">Smart-ATS Quantum Logic.</span>
                        </h1>
                        <p className="text-slate-400 text-lg leading-relaxed font-medium italic">
                            {hasCV 
                                ? "Your professional signature is indexed. Discover high-affinity matches detected by the neural core." 
                                : "Upload your profile metadata to activate semantic matching and predictive career analysis."}
                        </p>
                    </div>
                    ...
                    <div className="flex flex-col sm:flex-row gap-5 w-full md:w-auto relative z-10">
                        {!hasCV && (
                            ...
                            >
                                <FileText size={18} /> Initialize CV Node
                            </button>
                        )}
                        <button 
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                {[
                    { label: 'Neural Postulations', value: myApplications.length, icon: <FileText size={24} className="text-blue-400" />, color: 'bg-blue-400/10 border-blue-400/20' },
                    { label: 'Active Protocols', value: myApplications.filter(a => a.status === 'Interviewed').length, icon: <Target size={24} className="text-rose-400" />, color: 'bg-rose-400/10 border-rose-400/20' },
                    { label: 'Global Opportunities', value: jobs.length, icon: <Sparkles size={24} className="text-emerald-400" />, color: 'bg-emerald-400/10 border-emerald-400/20' },
                ].map((stat, i) => (
                    <motion.div 
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-10" id="explorer">
                ...
                <div className="lg:col-span-2 space-y-8">
                    <div className="flex items-center justify-between px-2">
                        <h2 className="text-sm font-black uppercase tracking-[0.3em] text-slate-500 flex items-center gap-3">
                            <Zap size={16} className="text-blue-500" /> Active Infrastructure
                        </h2>
                        <div className="flex items-center gap-2 text-blue-400 text-[10px] font-black uppercase tracking-widest cursor-pointer hover:text-blue-300 transition-colors group">
                            Semantic Feed <TrendingUp size={14} className="group-hover:translate-y-[-2px] group-hover:translate-x-[2px] transition-transform" />
                        </div>
                    </div>
                    ...
                    <div className="bg-white/5 backdrop-blur-3xl p-2 rounded-3xl border border-white/10 flex items-center shadow-2xl group focus-within:border-blue-500/30 transition-all">
                        ...
                        <input 
                            type="text" 
                            placeholder="Search role, location, or tech stack..." 
                        ...
                        />
                        <button className="bg-white/5 border border-white/10 text-white px-8 py-3 rounded-2xl font-bold text-[10px] uppercase tracking-widest hover:bg-white/10 transition-all shadow-xl backdrop-blur-md">Filters</button>
                    </div>
                    ...
                    <div className="space-y-6">
                        {filteredJobs.length === 0 ? (
                            <div className="bg-white/5 p-20 rounded-[3.5rem] border border-dashed border-white/10 text-center backdrop-blur-md">
                                ...
                                <p className="text-slate-500 font-medium italic">No nodes detected in the search stream.</p>
                            </div>
                                ...
                                <div className="flex flex-col md:flex-row gap-8 items-center md:items-start">
                                    ...
                                    <div className="flex-1 space-y-4 text-center md:text-left">
                                        <div className="flex flex-col md:flex-row md:items-center gap-3">
                                            ...
                                            {hasCV && (
                                                <span className="inline-flex items-center gap-1.5 bg-emerald-500/10 text-emerald-400 text-[9px] font-black px-3 py-1 rounded-full border border-emerald-500/20 shadow-lg shadow-emerald-500/5 uppercase tracking-widest self-center md:self-auto">
                                                    <Cpu size={10} /> High Affinity Node
                                                </span>
                                            )}
                                        </div>
                                        <div className="flex flex-wrap justify-center md:justify-start gap-5 text-[11px] font-bold text-slate-500 uppercase tracking-widest">
                                            ...
                                            <span className="flex items-center gap-2 bg-white/5 px-3 py-1.5 rounded-xl border border-white/5"><Clock size={14} className="text-slate-700" /> Active Track</span>
                                        </div>
                                    </div>
                ...
                <div className="space-y-8">
                    <h2 className="text-sm font-black uppercase tracking-[0.3em] text-slate-500 px-2">Neural Insights</h2>
                    <div className="bg-white/5 backdrop-blur-3xl p-10 rounded-[3.5rem] border border-white/10 shadow-2xl space-y-10 relative overflow-hidden">
                        ...
                        <div className="bg-slate-950 p-8 rounded-[2.5rem] border border-white/5 relative group overflow-hidden shadow-2xl">
                            ...
                            <h3 className="text-lg font-bold text-white mb-4 flex items-center gap-3 tracking-tight">
                               <div className="w-1.5 h-6 bg-blue-600 rounded-full"></div> Career Agent v5.0
                            </h3>
                            <p className="text-xs text-slate-400 leading-relaxed font-medium italic border-l-2 border-blue-500/30 pl-5">
                                {hasCV 
                                    ? "Your professional node is fully indexed. I recommend initializing protocols with 'High Affinity' nodes to maximize synchronization probability."
                                    : "CV metadata is currently missing. Initialize your CV node to activate my predictive matching engine and deep profile analysis."}
                            </p>
                        </div>

                        <div className="space-y-6 relative z-10">
                            <h4 className="text-[9px] font-black uppercase tracking-[0.3em] text-slate-600 border-b border-white/5 pb-3">Global Stream Updates</h4>
                            <div className="flex gap-5 items-start">
                                ...
                                <div>
                                    <p className="text-sm font-bold text-white tracking-tight">Market Expansion</p>
                                    <p className="text-xs text-slate-500 font-medium">+15% surge in high-performance React nodes detected this cycle.</p>
                                </div>
                            </div>
                            <div className="flex gap-5 items-start">
                                ...
                                <div>
                                    <p className="text-sm font-bold text-white tracking-tight">Neural Tip</p>
                                    <p className="text-xs text-slate-500 font-medium">Candidates with indexed portfolios show 40% faster synchronization.</p>
                                </div>
                            </div>
                        </div>
                    </div>
